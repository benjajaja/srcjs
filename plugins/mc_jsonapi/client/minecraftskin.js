srcjs.plugins.mc_jsonapi.minecraftskin = (function() {
...
$(document.head).append($('<script id="shader-fs" type="x-shader/x-fragment">').text('\n\
#ifdef GL_ES\n\
...
'));
$(document.head).append($('<script id="shader-vs" type="x-shader/x-vertex">').text('\n\
	...
	*/
	...
	function loadSkinFromFile(e) {
		...
		if (skinimporterchrome.files) {
			...
			reader.onload = (function(aImg, read) {
				return function(e) {
	...
	function init() {
		...
		if (skineditor && skineditor.getContext) {
			...
			console.log(skineditor, skineditor.getContext);
		...
		if (skinpreview && initGL( skinpreview ) != null) {
			...
			console.log(skinpreview, initGL( skinpreview ), skinpreview, fCanvasSupport);
			skinpreviewi = document.getElementById( 'skin-preview-img' );
			skinpreviewi.style.position = 'absolute';
		}
	...
	shitHole.append(skinEditor);
	
	var skinEditorMini = $('<canvas id="skin-editor-mini" name="image" width="64" height="32">')[0];
	shitHole.append(skinEditorMini);
